...
export default function Login() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    else setSuccess('Email enviado! Verifique sua caixa de entrada para redefinir a senha.')
    setLoading(false)
  }
        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          <div className="card" style={{ padding: '32px 28px' }}>
            ...
            <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0A0A0A', marginBottom: 4 }}>
              {mode === 'login' ? 'Entrar na sua conta' : mode === 'signup' ? 'Criar conta grátis' : 'Redefinir senha'}
            </h2>
            ...
            {mode === 'forgot' ? (
              <form onSubmit={handleForgotPassword} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {success && (
                  <div style={{ backgroundColor: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 10, padding: '12px 16px', fontSize: 13, color: '#16A34A' }}>{success}</div>
                )}
                ...
                <button type="button" onClick={() => switchMode('login')}
                  style={{ fontSize: 13, color: '#64748B', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', marginTop: 4 }}>
                  ← Voltar para o login
                </button>
              </form>
            ) : (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    {mode === 'login' && (
                      <button type="button" onClick={() => { setMode('forgot'); setError(''); setSuccess('') }}
                        style={{ fontSize: 12, color: '#2563EB', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                        Esqueci minha senha
                      </button>
                    )}
                  </div>
